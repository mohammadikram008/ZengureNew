import React, { Fragment } from 'react'
import {Link} from 'react-router-dom'
import './OurServices.css'
import  image22 from './image22.png'
import image34 from './image34.png'
import pencil_ruler from './pencil-ruler.png'
import arrow from './Arrow1.png'
import Footer from '../Footer/Index'
import logo from '../../assets/images/logo.png'
const Index = () => {
  return (
   <Fragment>
     <div className="services-page">
      <div className="div">
        <div className="overlap">
          {/* <div className="rectangle" /> */}
          {/* <img className="group" alt="Group" src="group-35076.png" /> */}
          <div className="ellipse" />
          <div className="ellipse-2" />
          <div className="ellipse-3" />
          <div className="rectangle-2" />
          <div className="rectangle-3" />
          <div className="rectangle-4" />
          <div className="frame-101">
            <div className="frame-2">
              <div className="text-wrapper">Our Services</div>
              <p className="p">
                Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin consepus vulputate
                Felis ornare
              </p>
            </div>
          </div>
          <div className="group-wrapper">
            <div className="group-2">
              <div className="frame-3">
                <div className="text-wrapper-2">Read More</div>
                <img className="arrow" alt="Arrow" src={arrow} />
              </div>
              <div className="text-wrapper-3">Graphic Designing</div>
              <p className="branding-design">
                Branding &amp; Design services are people’s view and the perception that makes your business visible as
                well as reliable.
              </p>
            </div>
          </div>
          <div className="div-wrapper">
            <div className="group-3">
              <div className="frame-4">
                <div className="text-wrapper-2">Read More</div>
                <img className="arrow" alt="Arrow" src={arrow} />
              </div>
              <div className="text-wrapper-4">Web Development</div>
              <p className="text-wrapper-5">
                At Zengur, we blend creativity and technology to craft websites that make a lasting impact.
              </p>
              <div className="group-4" />
            </div>
          </div>
          <div className="group-5">
            <div className="group-3">
              <div className="frame-4">
                <div className="text-wrapper-2">Read More</div>
                <img className="arrow" alt="Arrow" src={arrow} />
              </div>
              <div className="text-wrapper-6">SEO Consultancy</div>
              <p className="text-wrapper-5">
                Boost your online visibility with our SEO services. We optimize your website&#39;s content, technical
                aspects, and link
              </p>
              <div className="group-4" />
            </div>
          </div>
          <div className="ellipse-4" />
          <div className="pencil-ruler-wrapper">
            <img className="pencil-ruler" alt="Pencil ruler" src={pencil_ruler} />
          </div>
          <img className="image" alt="Image" src={image22} />
          <img className="img" alt="Image" src={image34} />
          <div className="overlap-wrapper">
            <div className="overlap-group">
              {/* <img className="vector" alt="Vector" src="vector-7.svg" /> */}
              <img className="image-2" alt="Image" src={logo} />
              <p className="text-wrapper-7">
                Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin
              </p>
              <div className="text-wrapper-8">@ Copyright 2023</div>
              <div className="frame-5">
               <div className="text-wrapper-9"><Link to='/' className='nav-item-link'>Home</Link> </div>
                <div className="text-wrapper-9"> <Link to='/ourservices' className='nav-item-link'>Services</Link></div>
                <div className="text-wrapper-9"> <Link to='/aboutus' className='nav-item-link'>AboutUs</Link></div>
                <div className="text-wrapper-9"> <Link to='/contactus' className='nav-item-link'>ContactUs</Link></div>
                <div className="text-wrapper-9"><Link to='/' className='nav-item-link'>FAQ</Link></div>
              </div>
            </div>
          </div>
        </div>
        <div className="overlap-2">
          <div className="rectangle-5" />
          <div className="rectangle-6" />
          <div className="group-6">
            <div className="frame-6">
              <div className="frame-7">
                <div className="text-wrapper-10">Our Services</div>
                <p className="text-wrapper-11">
                  Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin consepus vulputate
                  Felis ornare
                </p>
              </div>
              <div className="frame-8">
                <div className="text-wrapper-2">See More</div>
                <img className="arrow-2" alt="Arrow" src="arrow-3.svg" />
              </div>
            </div>
            <div className="overlap-group-2">
              <div className="group-7">
                <div className="group-8">
                  <div className="frame-9">
                    <div className="text-wrapper-2">Read More</div>
                    <img className="arrow-3" alt="Arrow" src="arrow-4-2.svg" />
                  </div>
                  <div className="rectangle-7" />
                  <div className="text-wrapper-12">Ui/Ux Designing</div>
                  <p className="text-wrapper-13">Felis ornare posuere dict nulla tempus vulput.</p>
                  <img className="group-9" alt="Group" src="group-30-2.png" />
                </div>
              </div>
            </div>
            <div className="group-10">
              <div className="frame-9">
                <div className="text-wrapper-2">Read More</div>
                <img className="arrow-4" alt="Arrow" src="arrow-4.svg" />
              </div>
              <div className="rectangle-7" />
              <div className="text-wrapper-12">Ui/Ux Designing</div>
              <p className="text-wrapper-13">Felis ornare posuere dict nulla tempus vulput.</p>
              <img className="group-9" alt="Group" src="group-30.png" />
            </div>
            <div className="group-11">
              <div className="frame-9">
                <div className="text-wrapper-2">Read More</div>
                <img className="arrow-5" alt="Arrow" src="image.svg" />
              </div>
              <div className="rectangle-7" />
              <div className="text-wrapper-12">Ui/Ux Designing</div>
              <p className="text-wrapper-13">Felis ornare posuere dict nulla tempus vulput.</p>
              <img className="group-9" alt="Group" src="image.png" />
            </div>
          </div>
          <div className="ellipse-5" />
          <div className="group-12">
            <div className="frame-10">
              <p className="felis-ornare-posuere">
                Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin consepus vulputate
                Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu&nbsp;&nbsp; There are many
                variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form,
                by injected humour,
              </p>
            </div>
            <div className="frame-11">
              <div className="text-wrapper-2">Read More</div>
              <img className="arrow-6" alt="Arrow" src="arrow-3-2.svg" />
            </div>
          </div>
          <div className="text-wrapper-14">About Us</div>
          <div className="group-13">
            <div className="image-3" />
            <img className="image-4" alt="Image" src="image-20.png" />
          </div>
          <div className="rectangle-8" />
          <div className="rectangle-9" />
          <div className="ellipse-6" />
          <div className="ellipse-7" />
          <div className="ellipse-8" />
          <div className="ellipse-9" />
          <div className="group-14">
            <div className="group-15">
              <div className="frame-10">
                <div className="text-wrapper-10">Our Process</div>
                <p className="text-wrapper-15">
                  Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin consepus vulputate
                  Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu
                </p>
              </div>
              <div className="frame-11">
                <div className="text-wrapper-2">Read More</div>
                <img className="arrow-7" alt="Arrow" src="arrow-3-3.svg" />
              </div>
            </div>
            <div className="overlap-3">
              <div className="rectangle-10" />
              <div className="rectangle-11" />
              <div className="text-wrapper-16">Design and Mockup Creation</div>
              <div className="frame-12">
                <div className="text-wrapper-2">Read More</div>
                <img className="arrow-8" alt="Arrow" src="arrow-3-4.svg" />
              </div>
              <div className="overlap-group-wrapper">
                <div className="overlap-group-3">
                  <div className="text-wrapper-17">Testing and Quality assurance</div>
                  <div className="frame-13">
                    <div className="text-wrapper-2">Read More</div>
                    <img className="arrow-9" alt="Arrow" src="arrow-3-5.svg" />
                  </div>
                </div>
              </div>
              <div className="group-16">
                <div className="overlap-group-3">
                  <div className="overlap-group-4">
                    <div className="text-wrapper-18">Development and Coding</div>
                    <div className="text-wrapper-18">Development and Coding</div>
                  </div>
                  <div className="frame-13">
                    <div className="text-wrapper-2">Read More</div>
                    <img className="arrow-10" alt="Arrow" src="arrow-3-6.svg" />
                  </div>
                </div>
              </div>
              <div className="text-wrapper-19">Development and Coding</div>
              <div className="group-17">
                <div className="overlap-group-3">
                  <div className="text-wrapper-20">Client feedback and revisions</div>
                  <div className="frame-13">
                    <div className="text-wrapper-2">Read More</div>
                    <img className="arrow-11" alt="Arrow" src="arrow-3-7.svg" />
                  </div>
                </div>
              </div>
            </div>
            <img className="pencil-ruler-2" alt="Pencil ruler" src="pencil-ruler-10037456-1.png" />
            <img className="image-5" alt="Image" src="image-22.png" />
            <img className="image-6" alt="Image" src="image-23.png" />
            <img className="image-7" alt="Image" src="image-24.png" />
          </div>
          <div className="frame-14">
            <div className="text-wrapper-2">See More</div>
            <img className="arrow-12" alt="Arrow" src="arrow-3-8.svg" />
          </div>
          <p className="text-wrapper-21">
            Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin consepus vulputate Felis
            ornare posuere dignissim
          </p>
          <div className="text-wrapper-22">Company Portfolio</div>
          <div className="ellipse-10" />
        </div>
        <img className="group-18" alt="Group" src="group-35081.png" />
        <img className="group-19" alt="Group" src="group-35061.png" />
        <div className="overlap-4">
          <div className="group-20">
            <div className="overlap-5">
              <div className="overlap-6">
                <div className="ellipse-11" />
                <div className="frame-15">
                  <img className="image-8" alt="Image" src="image-17.png" />
                  <div className="overlap-group-5">
                    <div className="frame-16">
                      <div className="text-wrapper-10">Contact Us</div>
                      <p className="text-wrapper-23">
                        Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin consepus vul
                      </p>
                    </div>
                    <div className="ellipse-12" />
                  </div>
                  <div className="overlap-7">
                    <div className="text-wrapper-24">Email</div>
                    <div className="rectangle-12" />
                    <div className="frame-17">
                      <div className="text-wrapper-25">Send</div>
                    </div>
                  </div>
                  <div className="ellipse-13" />
                  <div className="ellipse-14" />
                </div>
              </div>
              <div className="group-21">
                <div className="text-wrapper-26">FAQ</div>
                <div className="frame-wrapper">
                  <div className="frame-18">
                    <p className="text-wrapper-27">
                      Sit amet consectetur. Laoreet quis purus nunc. Montes lacus vitae gravida ut in.
                    </p>
                    <img className="group-22" alt="Group" src="group-51.png" />
                  </div>
                </div>
                <p className="text-wrapper-28">
                  Quisque dictum nunc eu porta elit nisl. Ut feugiat aliquet faucibus turpis curabitur ultrices dolor.
                  Lorem leo consectetur elementum nisl mauris egestas ultricies dui morbi.
                </p>
                <img className="vector-2" alt="Vector" src="vector-6.svg" />
                <div className="frame-19">
                  <div className="frame-18">
                    <p className="text-wrapper-29">Sit pellentesque tincidunt varius eu dolor. Diam.</p>
                    <img className="group-23" alt="Group" src="group-51-2.png" />
                  </div>
                  <img className="vector-3" alt="Vector" src="vector-6-2.svg" />
                </div>
                <div className="frame-20">
                  <div className="frame-18">
                    <p className="text-wrapper-29">Auctor volutpat scelerisque sed convallis congue.</p>
                    <img className="group-24" alt="Group" src="group-51-3.png" />
                  </div>
                  <img className="vector-4" alt="Vector" src="vector-6-3.svg" />
                </div>
                <div className="frame-21">
                  <div className="frame-18">
                    <p className="text-wrapper-29">
                      Lorem ipsum dolor sit amet consectetur. Posuere tincidunt urna faucibus viverra diam.
                    </p>
                    <img className="group-25" alt="Group" src="group-51-4.png" />
                  </div>
                  <img className="vector-5" alt="Vector" src="vector-6-4.svg" />
                </div>
                <div className="frame-22">
                  <div className="frame-18">
                    <p className="text-wrapper-29">
                      {" "}
                      Kllamcorpecongue nec gravida. Nunc ac varius bibendum tortor risus egestas volutpat.
                    </p>
                    <img className="group-26" alt="Group" src="group-51-5.png" />
                  </div>
                  <img className="vector-6" alt="Vector" src="vector-6-5.svg" />
                </div>
              </div>
            </div>
            <img className="image-9" alt="Image" src={logo} />
            <div className="overlap-8">
              <p className="text-wrapper-7">
                Felis ornare posuere dignissim dictum nulla tempus vulputate. Est facus eu proin
              </p>
              <div className="text-wrapper-8">@ Copyright 2023</div>
              <div className="frame-5">
                <div className="text-wrapper-9">Home</div>
                <div className="text-wrapper-9">Services</div>
                <div className="text-wrapper-9">About Us</div>
                <div className="text-wrapper-9">Contact Us</div>
                <div className="text-wrapper-9">FAQ</div>
              </div>
            </div>
            <img className="vector-7" alt="Vector" src="vector-7-2.svg" />
            <img className="image-10" alt="Image" src="image-3.png" />
          </div>
          <div className="ellipse-15" />
        </div>
        {/* <img className="ellipse-16" alt="Ellipse" src="ellipse-46.svg" /> */}
        {/* <img className="rectangle-13" alt="Rectangle" src="rectangle-49.svg" />
        <img className="rectangle-14" alt="Rectangle" src="rectangle-55.svg" />
        <img className="rectangle-15" alt="Rectangle" src="rectangle-50.svg" />
        <img className="rectangle-16" alt="Rectangle" src="rectangle-56.svg" />
        <img className="rectangle-17" alt="Rectangle" src="rectangle-51.svg" />
        <img className="rectangle-18" alt="Rectangle" src="rectangle-57.svg" /> */}
        {/* <div className="group-27">
          <div className="overlap-9">
            <div className="text-wrapper-30">Services</div>
            <div className="group-28">
              <div className="text-wrapper-31">Home</div>
              <div className="rectangle-19" />
            </div>
          </div>
          <div className="text-wrapper-32">About Us</div>
          <div className="frame-23">
            <div className="text-wrapper-9">Contact Us</div>
            <div className="text-wrapper-9">FAQ</div>
          </div>
          <div className="frame-24">
            <div className="text-wrapper-33">Request Quote</div>
          </div>
          <div className="group-29">
            <div className="overlap-group-6">
              <div className="text-wrapper-34">Zengur</div>
              <div className="text-wrapper-35">Zengur</div>
              <div className="text-wrapper-36">Zengur</div>
            </div>
          </div>
        </div> */}
      </div>
    </div>
    {/* <Footer/> */}
   </Fragment>
  )
}

export default Index